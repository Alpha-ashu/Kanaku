...
async function run() {
  const client = new Client({
    ...
    port: 5434,
    user: 'postgres',
    password: 'password', // let's try 'password' or check pg-testdata setup
    database: 'postgres',
  ...
  try {
    ...
    // Switch or query KANKUDB if it exists, otherwise query postgres
    const dbName = dbRes.rows.find(r => r.datname.toLowerCase().includes('kanku') || r.datname.toLowerCase().includes('finora') || r.datname.toLowerCase().includes('postgres'))?.datname || 'postgres';

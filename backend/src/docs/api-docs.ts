...
// The dynamic route-parsing generator that used to live here has been removed.
// It was module-local (nothing imported it) and entirely unreachable: its entry
// point, mergeOperations(), had no callers, so every helper below it — the route
// manifest, the parameter/response builders, the fs+path scanning — was dead
// weight shadowing the hand-written spec that is actually served.
...
// NOTE: mergeOperations() was removed — it built a merged OpenAPI operation map
// that nothing consumed. The served spec is assembled elsewhere in this file;
// this was a parallel, drifting copy.

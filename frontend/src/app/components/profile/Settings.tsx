...
export const Settings: React.FC = () => {
  ...
  const loadDevices = async () => {
    try {
      ...
      const res = await apiClient.get<any>('/devices', { showErrorToast: false });
      const list = res.data?.data || res.data || [];
      if (Array.isArray(list)) {
        setDevices(list);
      }
    } catch (err) {
      console.warn('[Settings] Failed to load devices:', err);
    } finally {
      setLoadingDevices(false);
    }
  };

  const handleDeactivateDevice = async (deviceId: string, deviceName: string | null) => {
    if (!confirm(`Revoke access for "${deviceName || deviceId}"? This device will be signed out and won't receive push notifications.`)) {
      return;
    }
    try {
      setRevokingDeviceId(deviceId);
      await apiClient.post(`/devices/${encodeURIComponent(deviceId)}/deactivate`);
  ...
  return (
    <CenteredLayout>
      <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto w-full px-2 sm:px-4">
        ...
        {/* ─── Registered Devices & Sessions Modal ──────────────────────── */}
        {showDevicesModal && (
            ...
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl border border-slate-200 overflow-hidden flex flex-col max-h-[85vh]">
              <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-900">Registered Devices</h3>
                    <p className="text-xs text-slate-400">Active sessions & push notification targets</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  >
                    <RefreshCw size={16} className={cn(loadingDevices && "animate-spin text-blue-600")} />
                  </button>
                  <button
                    type="button"
              ...
              <div className="p-4 sm:p-5 overflow-y-auto space-y-3 divide-y divide-slate-100">
                {devices.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm font-semibold text-slate-700">No registered devices found</p>
                    <p className="text-xs text-slate-400 mt-1">This browser session will register automatically</p>
                  </div>
                ) : (
                  devices.map((device) => {
                    const currentStoredId = localStorage.getItem('device_id') || localStorage.getItem('KANAKU_device_id');
                    const isCurrent = device.deviceId === currentStoredId;
                    const isDesktop = device.deviceType === 'desktop' || /windows|mac|linux/i.test(device.osType || '');
                    const isTablet = device.deviceType === 'tablet' || /ipad/i.test(device.deviceName || '');

                    return (
                      <div key={device.id || device.deviceId} className="pt-3 first:pt-0 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={cn(
                            "w-10 h-10 rounded-xl flex items-center justify-center shrink-0",
                            device.isActive ? "bg-slate-100 text-slate-700" : "bg-slate-50 text-slate-400"
                          )}>
                            {isTablet ? <Tablet size={18} /> : isDesktop ? <Monitor size={18} /> : <Smartphone size={18} />}
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">

...
export const VoiceAICommandCenter: React.FC<VoiceAICommandCenterProps> = ({
 ...
 merchant: cleanSpeechText(a.entities.merchant),
  ...
  const [activeTab, setActiveTab] = useState<'actions' | 'chat'>(() =>
    initialTab ?? (initialActions.some((a) => a.type === 'query') ? 'chat' : 'actions')
  );
 ...
 processActions();
  ...
  const recordableActionsCount = useMemo(() => {
    return actions.filter(a => a.type !== 'query').length;
  }, [actions]);
 ...
 <motion.div
  ...
  {/* Header Section */}
  <div className="p-4 md:p-8 pb-3 md:pb-4 border-b border-gray-100 shrink-0">
  ...
  {activeTab === 'chat' ? (
    <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
      <ConversationalAI
        ...
        initialQuery={
          initialActions.some((a) => a.type === 'query')
            ? transcript
            : undefined
        }

...
export const KaiScreen: React.FC = () => {
  ...
  return (
    <div
      ref={listRef}
      className="relative z-10 flex-1 flex flex-col max-w-md mx-auto w-full px-3 sm:px-4 overflow-y-auto scrollbar-none"
      style={{ paddingBottom: 'calc(var(--bottom-nav-height, 50px) + 20px)' }}
    >
      {/* Greeting */}
      <div className="pt-2 pb-1">
        <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight leading-tight">Hi, {name} 👋</h2>
        <p className="text-sm text-slate-600 font-medium">
          {visibleActions.length === 0 ? "I'm Kai. How can I help you today?" : 'How can I help you today?'}
        </p>
      </div>

      {/* Orb + status */}
      <div className="flex flex-col items-center py-3 sm:py-4 shrink-0">
        <div className="relative">
          <AIOrb size="lg" state={ORB_STATE[kai.state]} onClick={() => void kai.toggleListening()} showStatusGlow />
          {listening && (
            <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full bg-white/90 text-[10px] font-bold text-purple-700 shadow-xs border border-purple-100 whitespace-nowrap">
              Tap to stop
            </span>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, y: -4 }}
            className={`mt-4 max-w-full inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold shadow-xs ${
              kai.state === 'listening'
        ...
        {(kai.lastSay || kai.wrapUp) && (
          <p className="mt-2.5 text-center text-sm font-medium text-slate-700 px-4" data-testid="kai-say">
        ...
        <div className="mt-2 flex items-center gap-2">
          {canSpeak() && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-white/70 border border-white/80 text-slate-600 hover:text-purple-700 transition-colors cursor-pointer"
          ...
          {kai.offline && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-50 border border-amber-200 text-amber-700">
      ...
      <div className="relative w-full flex items-center bg-white/95 backdrop-blur-lg rounded-full px-2 py-1 border border-white shadow-[0_12px_32px_-4px_rgba(112,144,176,0.14)] shrink-0">
        <input
          type="text"
          value={draft}
        ...
        <button
          ...
          onClick={submitDraft}
          disabled={!draft.trim()}
          className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#8B5CF6] to-[#7C3AED] disabled:opacity-40 text-white flex items-center justify-center shadow-md shadow-purple-500/25 transition-all cursor-pointer active:scale-95 shrink-0"
          aria-label="Send"
        >
      ...
      <div className="mt-4 space-y-2.5">
        {visibleActions.length > 0 && (
          <div className="flex items-center justify-between px-1">
            <h3 className="text-xs font-black uppercase tracking-wide text-slate-500">Today's actions</h3>
            <span className="text-[11px] font-semibold text-slate-400">{savedToday} saved</span>
          </div>
        )}
        ...
        {visibleActions.length === 0 && (
          <div className="rounded-2xl border border-dashed border-purple-200 bg-white/60 px-4 py-4 text-center">
            <p className="text-xs font-semibold text-slate-600">
              Say things like <span className="text-purple-700">“I spent 2,000 on petrol”</span>,{' '}
              <span className="text-purple-700">“dinner with Arun and Jijo for 4,000”</span>,{' '}
              <span className="text-purple-700">“create a bike goal for 1.5 lakh”</span> or{' '}
              <span className="text-purple-700">“what's my food budget this week?”</span>
            </p>
          </div>
        )}
      </div>

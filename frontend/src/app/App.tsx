          ...
          <main className={`mobile-main flex-1 bg-transparent flex flex-col justify-start w-full ${currentPage === 'ai-assistant' ? '!h-full !max-h-full overflow-hidden !pb-0' : 'overflow-x-hidden mobile-safe-bottom'}`}>
            ...
            <PageErrorBoundary>
              <Suspense fallback={<PageLoader />}>
                <div
                  key={currentPage}
                  className={`page-view flex-1 flex flex-col w-full animate-in fade-in-50 duration-200 fill-mode-both ${currentPage === 'ai-assistant' ? '!h-full !min-h-0 !max-h-full overflow-hidden' : ''}`}
                >
                  {renderPage()}
                </div>
              </Suspense>

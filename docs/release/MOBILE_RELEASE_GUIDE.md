...
| **Voice speech-to-text** | ✅ | ❌ | ❌ | Web Speech API — see §7.1 |
| **SMS auto-detection** | ❌ | ✅² | ❌ | Custom `SmsDetectionPlugin` — see §4.1, §7.2 |
| Biometric unlock | — | ❌ | ❌ | Not implemented — see §7.3 |
| Push notifications | — | ❌ | ❌ | Not implemented — see §7.4 |
...
### 7.1 Voice speech-to-text does not work on native

`VoiceInput.tsx` uses the Web Speech API (`webkitSpeechRecognition`). It exists in Chrome
and Safari but **not** in Android WebView or iOS WKWebView. On device the UI detects this
and falls back to the manual text field with "Speech recognition not supported in this
browser. Please type below."

The microphone itself works (the waveform uses `getUserMedia`) — only the transcription
engine is missing. Closing this needs a native STT plugin
(`@capacitor-community/speech-recognition`) or streaming audio to a server-side STT. Both
are real feature work, not configuration, and are out of scope here.
...
### 7.3 No biometric unlock

`environment.ts` exposes a `BIOMETRIC_AUTH` feature flag and `types/index.ts` has a
`biometricEnabled` field, but no biometric plugin is installed and nothing reads the flag
to gate a real Face ID / fingerprint prompt. The app locks with a PIN on all platforms.
Implementing it means adding a biometric plugin plus `NSFaceIDUsageDescription`.

### 7.4 No push notifications

Only **local** notifications are wired. There is no FCM/APNs registration, no
`google-services.json`, and no server-side push. `app/build.gradle` already contains the
conditional `google-services` apply, so adding FCM later is mostly configuration.

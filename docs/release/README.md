# docs/release — Beta Release Audit Deliverables (2026-07-19)

Output of the full production-readiness audit. Start here:

1. **[BETA_READINESS_REPORT.md](BETA_READINESS_REPORT.md)** — verdict, acceptance-criteria
   evidence, bugs found/fixed, pre-launch actions.
2. [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) — actionable go-live checklist.
3. [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md) — what's intentionally partial at beta.

Reference set:

| Area | Doc |
|---|---|
| Architecture | [SYSTEM](SYSTEM_ARCHITECTURE.md) · [BACKEND](BACKEND_ARCHITECTURE.md) · [FRONTEND](FRONTEND_ARCHITECTURE.md) · [DATABASE](DATABASE_ARCHITECTURE.md) |
| API | [API_REFERENCE.md](API_REFERENCE.md) (index over docs/api/reference + OpenAPI) |
| Security | [SECURITY_AUDIT.md](SECURITY_AUDIT.md) · [RBAC_MATRIX.md](RBAC_MATRIX.md) |
| Quality | [TEST_PLAN.md](TEST_PLAN.md) · [TEST_RESULTS.md](TEST_RESULTS.md) · [CODE_QUALITY_REPORT.md](CODE_QUALITY_REPORT.md) · [UI_UX_REVIEW.md](UI_UX_REVIEW.md) · [dead-modules-inventory.txt](dead-modules-inventory.txt) |
| Performance | [PERFORMANCE_REPORT.md](PERFORMANCE_REPORT.md) |
| Ops | [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md) · [OPERATIONS_RUNBOOK.md](OPERATIONS_RUNBOOK.md) |
